...
class StochasticResult {
  float k, d;
  boolean isError;
  StochasticResult() { this.isError = true; }
  StochasticResult(float k, float d) { this.k = k; this.d = d; this.isError = false; }
}

class EmaResult {
  float value, distancePercent;  
  String trendDirection;  
  boolean isError;
  EmaResult() { this.isError = true; }
...
class InstrumentItem {
  ...
  InstrumentItem(String n, String t, String u, String type) {
    this.name = n; this.ticker = t; this.uid = u; this.type = type;
  }
  ...
  void drawItem(PApplet app) {
    ...
    app.fill(255); app.textSize(15);
    app.text(name + " (" + ticker + ")", x + 15, y + 27);
    app.fill(130); app.textSize(12);
