...
class TerminalView {
  ...
  private void drawSearchLayout() {
    app.fill(255); app.textSize(16); app.text("Поиск тикера:", 25, 40); // Твой лаконичный вариант!
  ...
  private void drawTableLayout() {
    ...
    // Шапка информационной таблицы
    app.textSize(13); app.fill(0, 160, 255); 
    app.text("Таймфрейм", 45, 132); app.text("Stochastic", 170, 132); app.text("Дистанция", 310, 132); app.text("Стратегия", 460, 132);
    app.fill(130, 145, 165); app.textSize(11); 
    app.text("(Интервал)", 45, 148); app.text("Lines %K / %D", 170, 148); app.text("до EMA " + emaPeriod, 310, 148); app.text("Mean Reversion", 460, 148);

    if (!isAllDataLoaded()) {
      app.fill(255, 204, 0); app.textSize(15); app.text("Загрузка индикаторов... Считаем Стохастик и скользящую...", 45, 200);
    } else if (timeframeMode == 0) {
      renderRow("30 Минут (30m)", tf30m, tf30mEma, 190); renderRow("15 Минут (15m)", tf15m, tf15mEma, 240); renderRow("5 Минут (5m)", tf5m, tf5mEma, 290);
    } else {
      renderRow("4 Часа (4h)", tf4h, tf4hEma, 190); renderRow("1 Час (1h)", tf1h, tf1hEma, 240); renderRow("30 Минут (30m)", tf30m, tf30mEma, 290);
    }
  ...
  private void renderRow(String title, StochasticResult stoch, EmaResult ema, float y) {
    ...
    if (stoch.isError || ema.isError) { app.fill(255, 70, 70); app.text("Ошибка расчета индикаторов", 170, y); return; }

    app.fill(240); String vals = app.nf(stoch.k, 1, 1) + " / " + app.nf(stoch.d, 1, 1); app.text(vals, 170, y);
    app.textSize(12);
    
    // Отрисовка динамических стрелок импульса
    if (stoch.k > stoch.d + 0.5f) { app.fill(0, 255, 150); app.text("▲", 178 + app.textWidth(vals), y); }
    else if (stoch.k < stoch.d - 0.5f) { app.fill(255, 50, 50); app.text("▼", 178 + app.textWidth(vals), y); }
    else { app.fill(150); app.text("◆", 178 + app.textWidth(vals), y); }
    
    app.textSize(14); String px = ema.distancePercent >= 0 ? "+" : "";
    app.fill(ema.distancePercent >= 0 ? app.color(255, 100, 100) : app.color(100, 255, 100));
    app.text(px + app.nf(ema.distancePercent, 1, 2) + "% (" + ema.trendDirection + ")", 310, y);

    // Торговые триггеры Mean Reversion стратегии
    if (stoch.k <= 20 && ema.distancePercent < -1.5f) { app.fill(0, 255, 150); app.text("BUY (Возврат вверх)", 460, y); }
    else if (stoch.k >= 80 && ema.distancePercent > 1.5f) { app.fill(255, 50, 50); app.text("SELL (Возврат вниз)", 460, y); }
    else { app.fill(150); app.text("Поиск паттерна...", 460, y); }
